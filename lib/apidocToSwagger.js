...
var { mockParser, mockDetailParser, requiredParser, commentParser } = require('./parser');
...
function translateInfoWithMock (type, value, mock) {
	...
	switch (type) {
		...
		case 'string':
			...
			let { _mock, _minLength, _maxLength, _enum } = mockDetailParser(mock)
			...
			if (info['mock']) {
				...
				info['mock'] = {mock: '@integer'}
			} else if (validator.isFloat(value)) {
				info['mock'] = {mock: '@float'}
			} else if (validator.isBoolean(value)) {
				info['mock'] = {mock: '@boolean'}
			} else if (validator.isIP(value)) {
				info['mock'] = {mock: '@ip'}
			} else if (validator.isUUID(value)) {
				info['mock'] = {mock: '@guid'}
			} else if (validator.isAfter('1970-01-01 00:00:00')) {
				info['mock'] = {mock: '@datetime'}
			} else if (validator.isDataURI(value)) {
				info['mock'] = {mock: '@imageData'}
			} else if (validator.isHexColor(value)) {
				info['mock'] = {mock: '@hex'}
			} else if (validator.isEmail(value)) {
				info['mock'] = {mock: '@email'}
			} else if (validator.isURL(value)) {
				info['mock'] = {mock: '@url'}
			} else {
				info['mock'] = {mock: '@string'}
			}
...
function scanTree (tree, comments) {
	if (tree instanceof AST.JsonString ||
		...
		let commentInfo = commentParser(comments, tree._key._position._start.line)
		let obj = {}
		obj[key] = Object.assign(value, {
			internalRequired: commentInfo.required,
			description: commentInfo.comment,
		}, translateInfoWithMock(value.type, tree._value.value, commentInfo.mock))
		return obj
	...
	if (verbs.success.examples instanceof Array && verbs.success.examples.length > 0) {
		...
		if (examples.type === 'json' || examples.type === 'json-original') {
			...
			if (content.indexOf('//') >= 0) {
				...
				let commentInfo = commentParser(content)
				return {
					type: "string",
					description: commentInfo.comment,
				}
