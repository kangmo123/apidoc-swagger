...
var sendToYapi = require('./sendToYapi')
...
function createApidocSwagger(options) {
    ...
    try {
        ...
        // send to yapi
        app.log.info('Checking url & token...');
        if (app.options.url && app.options.token) {
            app.log.info('Sending...');
            sendToYapi(app.options.url, app.options.token, api.swaggerData)
            .then(_ => {
                app.log.info('Done.');
            }).catch(e => {
                app.log.info(e);
